...
describe('game tests', () => {
    ...
    it('recognises a token has won #1', () => {
        const grid = [['x','x','x'],['-','-','-'],['-','-','-']];
        const t = new TykTacToe(3, grid);
        expect(t.doesDirectionHaveWinner(t.right, [0, 1], 'x')).toBe(true);
    });

    it('recognises a token has won #2', () => {
        const grid = [['-','-','o'],['-','o','-'],['o','-','-']];
        const t = new TykTacToe(3, grid);
        expect(t.doesDirectionHaveWinner(t.topRight, [1, 1], 'o')).toBe(true);
    });

    it('recognises a token has won #3', () => {
        const grid = [['-','-','-'],['-','-','-'],['o','o','o']];
        const t = new TykTacToe(3, grid);
        expect(t.doesDirectionHaveWinner(t.right, [2, 2], 'o')).toBe(true);
    });
